  ...
  MochaWeb.testOnly(function(){
    ...
    describe("Hypervideo", function(){
      ...
      it("should set hypervideo name", function() {
        var hypervideo = new Subject({owner: '1'});
        hypervideo.setName("new name");
        chai.assert.equal(hypervideo.name, "new name");
      });

      it("should be enabled hypervideo editing mode", function() {
        var hypervideo = new Subject({owner: '1'});
        chai.assert.isTrue(hypervideo.editing);
      });

      it("should disable hypervideo editing mode", function() {
        var hypervideo = new Subject({owner: '1'});
        hypervideo.setEditing(false);
        chai.assert.isFalse(hypervideo.editing);
      });
      ...
      it("should return subvideos list", function(){
        var hypervideo = new Hypervideo({owner: '1', subjectId:'1'});
        hypervideo.save();
